...
import { getWorkItems, createWorkItem, getClients, updateWorkItem, deleteWorkItem } from '../api';
...
const HomePage = () => {
  ...
  const handleEditSubmit = async (e) => {
    ...
    if (status === 3) {
      // If status is "Removed", delete the work item
      try {
        ...
        setEditModalIsOpen(false); // Close the modal after submit
      } catch (error) {
    ...
    try {
      ...
      // Update local state
      setBoards((prevBoards) => {
        // Remove the item from its current board
        let updatedBoards = prevBoards.map(board => ({
          ...board,
          items: board.items.filter(item => item.id !== selectedWorkItem.id)
        }));
        
        // Add the item to the new board
